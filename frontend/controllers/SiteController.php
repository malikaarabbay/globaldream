...
class SiteController extends Controller
{
    ...
    {
        $newsList = News::find()->where(['is_published' => '1'])->orderBy('created DESC')->limit(3)->all();
        
        $categories = Category::find()->where(['is_published' => '1', 'parent_id' => null])->orderBy('created DESC')->all();

        $products = Product::find()->where(['is_published' => '1', 'is_main' => '1'])->orderBy('created DESC')->limit(4)->all();
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            $model->saveFile();
            if ($model->save()){
                Yii::$app->session->setFlash('success', 'Спасибо. Отзыв успешно отправлено.');
            } else {
